...
class Tab(tk.Frame):
    def __init__(self,name,*args,**kwargs):
        ...
        if tab_type == 'results':
            print(height)
            self.board = Table(ctr_mid, width=mid_w, height=height, row_height=20)
        ...
        # Buttons
            ...
            self.pre = Table(ctr_mid, width=mid_w, height=height, row_height=20)
            # Buttons
            ...
            self.finish =  Table(ctr_mid, width=mid_w, height=height, row_height=20)
    #        inne.tree.bind("<Double-1>", self.onclick_out)
            # Tabell for de som er ute i skogen
            self.out =  Table(ctr_mid, width=mid_w, height=height, row_height=20)
    #        ute.tree.bind("<Double-1>", self.onclick_out)
            ...
            combo_races = TTK.Combobox(top_frame, width=30, values=list(zip(*self.db.races))[1])
            combo_races.grid(row=0, column=2, sticky='w')
            combo_races.bind("<<ComboboxSelected>>", self.set_class_buttons) 
...
#        # Lager knapper for hver klasse
        ...
        for class_name in self.race.class_names:
            self.buttons.append(tk.Button(self.adm_tab.ctr_left, text=class_name, command=partial(self.adm_tab.write_admin_list, class_name)).grid(row=i,column=j, padx = 10))
            i += 1
    ...
    def write_admin_list(self, class_name):
        ...
        if self.class_name:
            self.finish.after_cancel(self.res_tree_alarm)
        ...
        self.finish.tree.delete(*self.res.tree.get_children())
        result_list = self.race.make_result_list(class_name)
        self.write_table(result_list,'res')
        self.finish_tree_alarm = self.res.after(200, self.write_admin_list, class_name)
